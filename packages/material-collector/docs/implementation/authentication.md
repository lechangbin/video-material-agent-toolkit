...
- 每个平台配置位于 `<root>/<auth-profile>/<platform>/`。配置标识经过白名单校验，
  不能产生路径穿越。
- 每次探针、有头登录和登出都持有 `<auth-profile> + <platform>` 粒度的跨进程文件
...
- Windows 有头登录在进入等待循环前验证调用进程处于活动控制台会话，并确认可见
  Chrome 顶层窗口属于本次 `user-data-dir`、启动时间不早于本次有头启动且进程
  非 headless；其他 Chrome 窗口（包括同 profile 的旧进程）不能使验证通过。
  验证失败立即返回 `auth_desktop_unavailable`。
...
- 抖音和小红书先检查新导航页面的登录、登出和验证状态，再以自身份接口兜底。
  ...
  裸接口的 HTTP 406 不会覆盖页面中已明确显示的登录状态。
...
不能识别的响应保守返回 `probe_failed`。HTTP 401 判定无效，403、412、429 判定
需要人工挑战。接口或页面变化不会被伪装为空结果。

有头登录由调用方传入等待秒数。浏览器无法启动和等待超时分别转换为
`auth_desktop_unavailable` 与 `auth_login_timeout`，上层可将其持久化为
`auth_required` 检查点。
...
Chrome 直连启动参数，以及抖音和小红书页面登录态优先级。
